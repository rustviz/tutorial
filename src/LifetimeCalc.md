...
This lifetime can be explicitly named in the type. 
For example, `&'a int` is the type of a reference to an integer with named lifetime `'a` (pronounced ''tick a'' or ''a'' or ''alpha'').
...
```
The `print` function takes in a reference and returns nothing. Therefore, there is no need to worry about returning an invalidated reference. In this case, we don't need to annotated the lifetime explicitly for `s: &str`.
...
```
Even though `substr` returns a reference, but it only takes in one input reference `s: &str`. The borrow checker assumes that the lifetime of the returned reference `&str` must be constrained by `s: &str`.

More details on Rust's lifetime elision rules is in [https://doc.rust-lang.org/book/ch10-03-lifetime-syntax.html](the Rust book).
...
When you construct a `Book` value, Rust can infer the lifetime parameters using the same rules as for function application. For example, the borrow checker generates the constraints shown below for the shown construction of a `Book` value.
